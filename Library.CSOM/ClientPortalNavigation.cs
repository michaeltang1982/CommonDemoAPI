...
namespace Sierra.SharePoint.Library.CSOM
{
    ...
    /// Source: https://gist.github.com/vgrem/7090347
    /// </summary>
    public class ClientPortalNavigation
    {

        private readonly Web _web;
        private WebNavigationSettings _webNavigationSettings;
        ...
        {
            _web = web;
            _webNavigationSettings = new WebNavigationSettings(Context, _web);
        ...
        {
            //save web level flags first -- otherwise update does not work correctly
            _web.Update();
            Context.ExecuteQuery();

            TaxonomySession taxSession = TaxonomySession.GetTaxonomySession(Context);
            _webNavigationSettings.Update(taxSession);
            _web.Update();
            Context.ExecuteQuery();
        }
        ...
        public bool InheritGlobalNavigation
        {
            set
            {
                _webNavigationSettings.GlobalNavigation.Source = (value) ? StandardNavigationSource.InheritFromParentWeb : StandardNavigationSource.PortalProvider;
            }
        }

        public bool InheritCurrentNavigation
        {
            set
            {
                _webNavigationSettings.CurrentNavigation.Source = (value) ? StandardNavigationSource.InheritFromParentWeb : StandardNavigationSource.PortalProvider;
            }
        }
